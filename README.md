...
|-----------------------|-------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------|---------|
| bam-path              | -b    | The bam file(s) to be visualized. Accepts local paths or `http(s)`/`ftp` URLs, like samtools; remote files need an index (`.bai`/`.csi`) alongside them. When multiple bam files are given they will get concatenated vertically. |         |
